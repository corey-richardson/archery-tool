...
export default function RecordsManagement({ scores }: { scores: Score[] }) {
    const columns: GridColDef[] = [
...

        {
            ...
            sortable: false,
            renderCell: (params) => {
                const score = params.row as Score & { roundType: string, roundClassification: string, id: string };
...

        {
            ...
            sortable: false,
            renderCell: (params) => {
                const score = params.row as Score & { id: string };
                const [handicap, setHandicap] = useState<number | ''>(params.value ?? '');

                const handleBlur = async () => {
                if (handicap === params.value) return; // no change

                try {
                    const response = await fetch(`/api/scores/${score.id}/handicap`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
...

        {
            ...
            sortable: false,
            renderCell: (params) => {
                const score = params.row as Score & { id: string, processedAt?: Date };
